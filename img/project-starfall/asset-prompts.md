...
Canonical forward art direction: **Playful Starlit Frontier Fantasy**. Future Project Starfall assets should use clean 2D fantasy art with painterly side-scroller backgrounds, compact 3.5-to-4-head adventurers, cel-shaded sprite assets, crisp contour outlines, readable silhouettes, luminous fallen-star magic, guild-town warmth, blue-and-gold Starfall motifs, portals, runes, lanterns, crystal inlays, and practical crafted materials. Professional polish must come from stronger silhouettes, animation timing, value grouping, and cohesive worldbuilding—not realistic adult anatomy, grim tactical equipment, muddy palettes, or dense cinematic texture. Historical prompt summaries below may describe earlier passes, but replacement assets should follow the canonical style in the GDD and must remain original rather than imitating any external game's characters or assets.
...
Playable player portraits and runtime player animation sheets are source-backed again. The approved compact-adventurer proportion study is stored at `asset-sources/project-starfall/players/starfall-chibi-equipment-reference.png`, and the first plain adventurer in that lineup is the current base direction. Treat it as a proportion and equipment-readability study only: future designs must use original Starfall-specific faces, clothing, tools, and animation. Keep `build/process-project-starfall-player-ai-assets.js` or `build/generate-project-starfall-action-sprites.js --only players` as the canonical refresh path for `img/project-starfall/characters/` and `img/project-starfall/animations/players/`. Equipment layer visual redesign is intentionally deferred until the base character and animation are approved.
