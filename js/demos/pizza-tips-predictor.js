/* Browser calculation extracted from the preserved Lambda implementation.
 * Keep numerical behavior aligned through pizza-tips-runtime.test.js. */
(function (root, factory) {
  if (typeof module === 'object' && module.exports) module.exports = factory;
  else root.createPizzaTipsPredictor = factory;
})(typeof globalThis !== 'undefined' ? globalThis : this, function (model, CONFIDENCE_LEVEL) {
  'use strict';
  ...
  const predictionResult = (statusCode, data) => ({ statusCode, data });

  const predict = (payload) => {
    if (!payload || typeof payload !== 'object') {
      return predictionResult(400, { error: 'Invalid payload.' });
    }
    ...
    if (errors.length) {
      return predictionResult(400, { error: 'Missing or invalid inputs.', details: errors });
    }
    ...
    if (!city) {
      return predictionResult(400, { error: 'Location is outside supported city boundaries.' });
    }
    ...
    return predictionResult(200, response);
  };

  return { predict, resolveCity };
});
