...
| File Transcriber | Authenticated / Vercel API + S3 + Amazon Transcribe + DynamoDB ledger | File preflight, cost estimate/approval, direct upload, deterministic job start, adaptive status polling, transcript display/copy, bulk retry recovery, private transcript history, daily usage, notifications, and metadata-only generic session capture. | Fails closed in production without the usage ledger; atomically reserves per-user daily cost/files/concurrency and global daily cost/files; normally reserves the provider’s 28,800-second maximum cost, while explicitly trusted owner subjects use their signed browser estimate; uses exact-size/content-type tagged presigned POSTs; makes start idempotent; returns a recovery token for ambiguous starts; safely refunds only confirmed missing jobs; retries transient starts/polls; adapts polling cadence; resumes one or all recoverable jobs sequentially; shows authenticated UTC-day usage; stores filename plus transcript text in a dedicated, user-scoped, lazy-loading 90-day history while keeping media temporary; preserves only expiring, subject-scoped recovery metadata in same-tab session storage across reloads; constrains transcript fetch time, redirect hosts, raw bytes, and final response bytes; and cleans uploaded media on terminal paths. | **P1:** add scheduled delayed deletion of retained Transcribe job records after the run-token retention window, plus durable retries for failed media cleanup. Replace both pessimistic and owner-trusted duration reservations with authoritative server-side duration extraction when a dependable parser is available. **P2:** add a deployment-wide concurrency cap and a durable presign/start request limiter if observed demand warrants them. |
...
- Generic Transcriber sessions store workflow metadata, not transcript bodies. Transcript bodies live only in the dedicated private history store with separate user-scoped APIs and 90-day retention.
...
- Transcribe history text: `tool=amazon-transcribe` and `retention=history`; private S3 text and its DynamoDB index expire after 90 days by default.
...
- [ ] Resume all processes every recoverable job sequentially without duplicate starts or charges.
...
- [ ] Authenticated usage shows the current reserved UTC-day amount out of $100 and updates after reservations and terminal reconciliation.
- [ ] History list/detail/delete stays within the authenticated subject; only filename plus transcript text persists, media is absent, cursors are opaque, and both metadata and S3 text expire after 90 days.
- [ ] Browser completion/partial/failure notifications fire once only after explicit permission and clearly disclose that the page must remain open.
- [ ] Terminal paths delete media; temporary-media lifecycle removes any forced orphan while the separate history lifecycle retains text for 90 days. Future delayed job deletion waits beyond the run-token retention window.
...
| P1 | Replace pessimistic Transcribe duration reservation with authoritative server duration | File Transcriber | Medium: even the current $2.88-per-file fallback reservation can limit legitimate short jobs; trusted owner subjects temporarily use signed browser estimates. | L | Server-derived duration cannot be client-forged, preserves atomic budget enforcement, and has fallback behavior for unsupported containers. |
