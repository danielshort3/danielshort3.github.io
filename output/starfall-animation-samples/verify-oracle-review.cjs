...
async function run() {
  ...
  try {
    for (const width of widths) {
      ...
      await page.frameLocator('iframe').locator('#oracle-cast-review[data-ready="true"]').waitFor({ timeout: 15000 });
      let frame = null;
      for (const candidate of page.frames()) {
        if (candidate.parentFrame() && await candidate.locator('#oracle-cast-review').count()) frame = candidate;
      }
      assert(frame, 'The rendered visualization must exist inside its iframe');
      const root = frame.locator('#oracle-cast-review');
      ...
      assert.strictEqual(await root.locator('[data-after-label]').innerText(), data.afterLabel || data.after.label || 'Bloom cast study');
      assert.strictEqual(await root.locator('[data-title]').textContent(), typeof data.title === 'string' ? data.title : 'Icebloom Oracle · cast study');
      ...
      await page.screenshot({ path: path.join(outputDirectory, `oracle-review-${width}.png`), fullPage: true });
  ...
  fs.writeFileSync(path.join(outputDirectory, 'oracle-review-verification.json'), JSON.stringify(report, null, 2));
