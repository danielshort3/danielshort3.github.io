...
function renderHeader({ settings, navigation, projectsById, audienceLabel }) {
  ...
  return [
    ...
    `                <div class="nav-dropdown-header" aria-hidden="true">${escapeHtml(portfolio.header || 'Top 5 Projects')}</div>`,
    ...
    '            <span class="nav-link-caret" aria-hidden="true"></span>',
    '          </a>',
    `          <div class="nav-dropdown" id="nav-dropdown-resume" aria-label="${escapeHtml(resume.ariaLabel || 'Resume download')}">`,
    ...
    '            <span class="nav-link-caret" aria-hidden="true"></span>',
    '          </a>',
    '          <div class="nav-dropdown nav-dropdown-contact" id="nav-dropdown-contact" aria-label="Contact options">',
    ...
    '          </div>',
    '        </div>',
    `        <form class="nav-search" action="${escapeHtml(trimLeadingSlash(search.action || 'search'))}" method="get" role="search">`,
...
function renderHead({ settings, page }) {
  ...
  return [
    ...
    `  <meta name="theme-color" content="${escapeHtml(page.themeColor || settings.themeColor || '#0D1117')}">`,
    stylesheetLines,
