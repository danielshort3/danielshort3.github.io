...
| Short Links | Admin / Vercel API + DynamoDB | Link/set CRUD, redirects, expiration, click history, export, health checks, and destination tests. | Atomically reserves lowercase slug identities, canonicalizes click keys, prevents click updates from creating phantom records, clears stale expiration on permanent links, bounds JSON bodies, adds opaque storage cursors, minimizes click telemetry, applies 90-day click TTL metadata, exposes TTL status in health, and hardens outbound destination tests against private addresses, redirect pivots, and DNS rebinding by pinning validated addresses. | **P0:** resolve lowercase duplicates and backfill reservation records for every existing link before enabling the new write path; grant transaction/TTL-description IAM and enable TTL on `expiresAt`. **P1:** add a queryable index/materialized view for filtered/sorted listings so large catalogs do not require full scans; audit pre-change click records for fields no longer retained. |
...
- Short-link click events: `expiresAt`, 90 days.
...
2. **Short Links:** grant `dynamodb:TransactWriteItems` for reservation/write/delete transactions and `dynamodb:DescribeTimeToLive` for health. Enable TTL on the clicks table’s `expiresAt` attribute.
...
3. **Short-link click privacy:** decide whether pre-change rows containing full referrers or coordinates should be removed immediately rather than merely waiting for TTL.
...
- [ ] Click telemetry omits query strings, full referrers, latitude, and longitude; TTL health reports `ENABLED` on `expiresAt`.
