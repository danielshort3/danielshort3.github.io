(() => {
  ...
  const buildResumePrompt = (context = {}) => {
    ...
    return [
      ...
      '3. Portfolio website: danielshort.me. This site lists my projects and highlights selected results (e.g., hours saved, traffic lift, and an RL puzzle solver demo). Use all relevant portfolio projects when they strengthen the resume, but include no more than one pizza-related project so the projects section does not feel repetitive. It also includes sections on certifications, degrees and demonstrated strengths (Python, SQL & ETL, Tableau dashboards, data wrangling, reinforcement learning, regression analysis).',
      '',
      ...
      '- Rewrite the professional summary to highlight my most relevant experience, drawing on the website\'s metrics (e.g., 750% traffic lift, 200+ hours saved) and emphasizing skills that match the job description.',
      ...
      '- Expand the projects section by selecting the most relevant portfolio projects from danielshort.me based on the job description. Use all relevant projects that strengthen the resume, but include no more than one pizza-related project so the section does not feel repetitive. For each included project, describe the problem, approach, and outcome using the descriptions from the website.',
  ...
  const buildCoverLetterPrompt = (context = {}) => {
    ...
    return [
      ...
      '',
      'Portfolio website:',
      'danielshort.me. Use relevant portfolio projects from the website when they strengthen the letter, but include no more than one pizza-related project so the examples do not feel repetitive.',
      '',
      ...buildNaturalLanguagePrompt('A tailored, one-page cover letter.'),
      '',
      'Instructions:',
      '1. Refer to the attached resume file and danielshort.me to identify my key roles, achievements, skills, and relevant portfolio projects.',
  ...
  const authIsValid = (auth) => {
    if (window.ToolsAuth && typeof window.ToolsAuth.authIsValid === 'function') {
      return window.ToolsAuth.authIsValid(auth);
    }
    return false;
  };
  ...
  const ensureFreshAuth = async () => {
    ...
    try {
      return await window.ToolsAuth.ensureFreshAuth();
    } catch {
      return null;
    }
  };
  ...
  const getSignedInLabel = (auth) => {
    if (window.ToolsAuth && typeof window.ToolsAuth.getUser === 'function') {
  ...
  const startAuthWatcher = () => {
    ...
    authExpiryTimer = window.setTimeout(async () => {
      await syncAuthState();
    ...
    if (els.jumpTabButtons.length) {
	  ...
	  const updateAuthUI = () => {
	    ...
	    if (els.authStatus) {
	      if (authed) {
	        clearAuthMessage();
	        const label = getSignedInLabel(state.auth) || 'Signed in';
  ...
  const initDashboardInteractions = () => {
    ...
    document.addEventListener('keydown', (event) => {
      ...
      if (window.ToolsAuth && typeof window.ToolsAuth.handleRedirect === 'function') {
        try {
          await window.ToolsAuth.handleRedirect();
        } catch (err) {
      ...
      document.addEventListener('tools:auth-changed', async () => {
        await syncAuthState();
        if (authIsValid(state.auth)) {
          clearAuthMessage();
          updateAuthUI();
