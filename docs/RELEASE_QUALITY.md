...
The Lighthouse harness adds gzip delivery for compressible responses to reflect the production CDN, blocks external services and all API calls, and measures `/`, `/tools/text-compare`, `/portfolio/website` and `/minesweeper-demo`. Raw reports include request timings, transferred bytes and the browser's benchmark index. The checked baseline gates CLS at 0.1 and LCP/TBT regressions at 20% plus a 250ms/50ms noise floor respectively. CI builds the pinned `referenceCommit` from the reviewed baseline and measures it alongside the candidate on the same runner. CI compares `reviewed TBT + median(candidate TBT - reference TBT)` to the unchanged reviewed TBT limit; LCP and CLS retain their absolute gates. The summary records raw medians, paired differences, adjusted TBT and limits, including when a regression fails. A missing or unbuilt reference fails the check. Local `npm run performance:lab` keeps the raw baseline comparison when no reference is supplied. The 2.5-second LCP target remains a goal, not a claim that this local site currently meets it.

Update a baseline only from a committed source revision with explicit `node build/measure-lighthouse.cjs --update-baseline`. It records that revision as `referenceCommit`. Inspect all three runs and medians, review the baseline diff, and commit the reviewed baseline separately. Do not update the baseline or its reference commit to hide a regression.
