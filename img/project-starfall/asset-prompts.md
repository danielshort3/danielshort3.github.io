...
Canonical forward art direction: **Starlit Frontier Fantasy**. Future Project Starfall assets should use clean AI-generated 2D fantasy art with painterly side-scroller backgrounds, cel-shaded sprite assets, crisp contour outlines, readable silhouettes, luminous fallen-star magic, guild-town warmth, blue-and-gold Starfall motifs, portals, runes, lanterns, crystal inlays, and practical crafted materials. Historical prompt summaries below may describe earlier passes, but replacement assets should follow the canonical style in the GDD.
...
Updated on 2026-06-07 with `build/process-project-starfall-player-ai-assets.js`.

Playable player portraits and runtime player animation sheets are source-backed again. The selected MapleStory-like chibi reference is stored at `asset-sources/project-starfall/players/starfall-chibi-equipment-reference.png`, and the first plain adventurer in that lineup is the current approved base direction. Keep `build/process-project-starfall-player-ai-assets.js` or `build/generate-project-starfall-action-sprites.js --only players` as the canonical refresh path for `img/project-starfall/characters/` and `img/project-starfall/animations/players/`. Equipment layer visual redesign is intentionally deferred until the base character and animation are approved.
...
The source-backed full-map WebP backdrops now use full-width seamless wrap blending instead of reflected edge synthesis or half-tile repetition. Each output keeps the original generated image through the center, blends in a half-width circular offset only near the outer edges, and locks the final edge columns so the left and right sides meet cleanly when the renderer draws adjacent copies. The processor does not use x-axis flipping, flopping, negative scale, or reversed sampling. This applies to Starfall Crossing, Greenroot Meadow, Thornpath Thicket, Bramble Depths, Rustcoil Ruins, Gearworks Vault, Cinder Hollow, Emberjaw Lair, Bandit Ridge Camp, Oreback Quarry, Ashglass Pass, Stormbreak Cliffs, Astral Archive, Eclipse Frontier, and Endless Rift.
...
Player sheets: 6 columns by 9 action rows for idle, run, jump, fall, basic attack, skill, party buff, hit, and defeat. Built from the existing character sprites with deterministic frame offsets, squash, tilt, and localized action overlays so every class shows a readable action pose.
...
Updated on 2026-06-07 with the reference-matched pose-rig plain adventurer pass in `build/process-project-starfall-player-ai-assets.js`.

The base playable character now derives from the first plain adventurer in `asset-sources/project-starfall/players/starfall-chibi-equipment-reference.png`, not from procedural block shapes. The extracted transparent source lives at `asset-sources/project-starfall/players/plain-adventurer-base.png`, and the processor uses that source directly for the upright three-quarter idle while keeping the approved reference head/hair on deterministic action poses. The review sheet at `asset-sources/project-starfall/players/plain-adventurer-review.png` compares the reference crop, generated idle, runtime-scale idle, starter-gear preview, and every animation row. Current class portrait and animation paths are all regenerated from this same approved starter base so selecting a class cannot fall back to the old blocky art.

The standard runtime player contract remains a 6 by 10 grid of 160px frames ordered as idle, run, jump, fall, climb, basic, skill, party, hit, and defeat. Player source sheets use the shared `#00ffff` guide grid with transparent cells so rotated animation frames do not pick up chroma-key fringe. The player validator now requires semantic action rows: planted subtle idle, alternating run stride, jump lift/landing, falling brace, climb reach/pull, attack windup/strike, skill charge/cast/release, party buff channel, hit recoil, and defeat collapse. Existing equipment sheets are left unchanged until the base character and animation direction are accepted.

Updated on 2026-06-08 with the connected player animation pass in `build/process-project-starfall-player-ai-assets.js`.

The source-backed player processor keeps the approved base character as a connected full-body silhouette for non-idle action rows. Earlier sliced-limb articulation proved too unstable because arms and legs could detach or move in unreadable directions, so run, jump, fall, climb, basic attack, skill cast, party/buff cast, hit, and defeat now use restrained connected-body pose changes plus action-specific overlays. Melee, bow, and magic rows keep the same 6 by 10 grid but use different weapon/cast overlays and stricter validation for edge clipping. The runtime rig also exposes named attachment anchors plus per-frame anchor resolution for future equipment overlays. Existing equipment layer sheets still share the grid and should be regenerated in a later pass once the final base action poses are approved.
...
All playable class portraits and animation sheets are now regenerated as unique deterministic class variants instead of pointing to the shared generic fallback. The variants preserve the same 6 by 10 160px rig and equipment alignment while adding class palettes and signature gear or motifs for Fighter, Mage, Archer, Guardian, Berserker, Duelist, Fire Mage, Rune Mage, Storm Mage, Sniper, Trapper, and Beast Archer.
...
The town landmark atlas is deterministic and cell-aligned, not externally sourced. It writes `img/project-starfall/environment/structures/town-landmarks.png` as a 4 by 2 grid of 256px transparent sprites for Starfall guild hall, Rustcoil workshop, Cinder forge, Frostfen lodge, Stormbreak gate, Astral observatory, market awning, and lantern arch. The runtime composes these behind safe-zone streets and stations so towns read as fuller MapleStory-style hubs without adding collision or blocking combat silhouettes.

## AI Enemy Sprite Sheet Pipeline

Updated on 2026-05-25 with `build/process-project-starfall-compact-bandits.js`.

Enemy visuals are now treated as compact source sprite sheets instead of outputs from the deterministic blocky sprite generator. The runtime contract is one transparent `384x1024` PNG per enemy under `img/project-starfall/animations/enemies/`, laid out as a 3 by 8 grid of 128px frames for idle, move, telegraph, attack, projectile, buff, hit, and defeat. The hit row uses only the first frame at runtime; every other row uses three frames. Raw generated compact sheets belong in `asset-sources/project-starfall/enemies/compact/` as `<enemy-file-id>-compact-source.png` and are processed into final `*-compact-sheet.png` files plus 320px portraits with the compact sheet processor. When a new raw compact source is not available, the processor can only migrate a previous finalized `*-sheet.png` into the compact runtime format; it no longer has a procedural enemy-art refresh path.

Shared prompt direction: one original charming 2D side-scroller MMO monster per sheet, clean dark outline, rounded readable silhouette, soft cel shading, simple expressive animation, consistent proportions and markings across all 24 source cells, right-facing only, no UI/text/watermark, no copied MapleStory characters or assets. Use the shared `#00ffff` bordered-cell grid and a perfectly flat chroma-key background, usually `#00ff00` and `#ff00ff` for green/plant/ooze subjects, with no shadows, gradients, floor plane, or texture.
