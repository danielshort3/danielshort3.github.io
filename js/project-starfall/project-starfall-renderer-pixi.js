(function initProjectStarfallPixiRenderer(global) {
  ...
  class ProjectStarfallPixiRenderer {
    ...
    async init() {
      ...
      this.setupScene();
      this.primeTextures(this.assetPaths);
      this.ready = true;
      this.setActive(true);
      return true;
    }
    ...
    renderBackground(snapshot) {
      ...
      for (let x = startX; x < width + MAP_BACKGROUND_TILE_OVERLAP_PX; x += drawWidth) {
        this.drawTexture('background', texture, x, 0, tileWidth, height, {
          anchorX: 0,
          anchorY: 0
        });
      }
    ...
    renderProceduralBackground(snapshot, width, height) {
      ...
      for (let i = 0; i < 7; i += 1) {
        ...
        graphics.ellipse(x, 86 + (i % 3) * 40, 80, 22).fill({ color: 0xfff2b4, alpha: 0.38 });
      }
      for (let i = 0; i < 6; i += 1) {
        const x = positiveModulo(i * 260 - cameraX * 0.34, width + 260);
        graphics
          .moveTo(x, height)
          .lineTo(x + 150, 230 + (i % 2) * 40)
          .lineTo(x + 320, height)
          .closePath()
    ...
    drawEnvironmentCell(kind, profile, cellIndex, x, y, w, h, options) {
      const texture = this.getEnvironmentCellTexture(kind, profile, cellIndex);
    ...
    drawEnvironmentTileStrip(snapshot, kind, profile, cellIndex, x, y, w, h, seed, alternateCellIndex) {
      const asset = this.getEnvironmentAsset(kind, profile);
      ...
      let drew = false;
      for (let tx = x + firstOffset * tileW; tx < x + w && tx < rightLimit; tx += tileW) {
        const drawW = Math.min(tileW, x + w - tx);
        if (tx + drawW < leftLimit) continue;
        const useAlt = alternateCellIndex != null && seededUnit(seed, Math.round(tx / tileW)) > 0.68;
        if (this.drawEnvironmentCell(kind, profile, useAlt ? alternateCellIndex : cellIndex, tx, y, drawW, h)) drew = true;
      }
      return drew;
    }
    ...
    renderTownStructureEntry(snapshot, runtime, entry, groundY) {
      ...
      const y = groundY - h + Number(entry.footOffset || 0);
    ...
    drawPlatformThemeTrim(graphics, map, platform, index) {
      ...
      if (theme.includes('cinder') || theme.includes('ember') || theme.includes('fire')) {
        ...
        for (let x = platform.x + 28; x < platform.x + platform.w - 24; x += 84) {
          graphics.rect(x, platform.y - 2, 18, 4).fill({ color: 0x8ec878, alpha: 0.44 });
        }
      }
    }
    ...
    drawTiledPlatformTerrain(snapshot, map, platform, index) {
      ...
      const topY = platform.y - (index === 0 ? 16 : 13);
      const topH = index === 0 ? 28 : 24;
      const bodyY = platform.y + 7;
      const bodyH = Math.max(20, platform.h + (index === 0 ? 34 : 14));
      this.drawEnvironmentTileStrip(snapshot, 'terrain', profile, cells.body, platform.x, bodyY, platform.w, bodyH, seed);
      this.drawEnvironmentTileStrip(snapshot, 'terrain', profile, cells.top, platform.x, topY, platform.w, topH, seed, cells.topAlt);
      this.drawEnvironmentTileStrip(snapshot, 'terrain', profile, cells.underside, platform.x, platform.y + platform.h - 1, platform.w, 14, `${seed}:under`);
      if (isRectInBounds({ x: platform.x - 18, y: topY, w: 36, h: bodyH + topH * 0.5 }, snapshot.bounds, 80)) {
        this.drawEnvironmentCell('terrain', profile, cells.left, platform.x - 18, topY, 36, bodyH + topH * 0.5);
      }
      if (isRectInBounds({ x: platform.x + platform.w - 18, y: topY, w: 36, h: bodyH + topH * 0.5 }, snapshot.bounds, 80)) {
        this.drawEnvironmentCell('terrain', profile, cells.right, platform.x + platform.w - 18, topY, 36, bodyH + topH * 0.5);
      }
      if (platform.w > 180 && seededUnit(seed, 'cap') > 0.38) {
        const detailX = platform.x + Math.floor(platform.w * (0.22 + seededUnit(seed, 'detail') * 0.56));
        if (isRectInBounds({ x: detailX, y: platform.y - 8, w: 54, h: 22 }, snapshot.bounds, 80)) {
          this.drawEnvironmentCell('terrain', profile, cells.detail, detailX, platform.y - 8, 54, 22, { alpha: 0.85 });
        }
      }
    ...
    renderWorldEffects(snapshot) {
      ...
      (snapshot.worldEffects || []).forEach((effect) => {
        ...
        if (type === 'lootPickup') {
          const rise = Math.sin(now * 5 + x * 0.01) * 4;
          const radius = Math.max(5, Number(effect.r || 12));
          this.drawShape('vfx', 'glow', x, y + rise, radius * 2.8, radius * 2.8, { tint: color, alpha: 0.24 });
          this.drawShape('vfx', 'circle', x, y + rise, radius * 0.92, radius * 0.92, { tint: color, alpha });
          return;
        }
    ...
    renderProjectiles(snapshot) {
      ...
      (snapshot.projectiles || []).forEach((projectile) => {
        ...
        this.drawLine('vfx', centerX, centerY, centerX - vx * 0.035, centerY - vy * 0.035, Math.max(2, radius * 0.54), {
          tint: color,
          alpha: 0.36
        });
    ...
    renderLoot(snapshot) {
      ...
      (snapshot.lootDrops || []).forEach((drop) => {
        ...
        const texture = this.getTexture(drop.itemAsset);
        if (texture) {
    ...
    renderParty(snapshot) {
      (snapshot.partyMembers || []).forEach((member) => {
        ...
        if (!this.renderActorSprite(member, box, 1) && !this.renderActorRig(member, box, 1)) {
          this.frameStats.actorFallbacks += 1;
    ...
    renderPlayer(snapshot) {
      ...
      const equipmentFrames = (player.equipmentLayers || []).map((layer) => layer && layer.frame).filter(Boolean);
      const compositeKey = [player.animationFrame].concat(equipmentFrames).map((frame) => this.getFrameTextureKey(frame)).filter(Boolean).join('|');
      const compositeTexture = player.animationFrame && equipmentFrames.length
        ? this.getCompositeFrameTexture([player.animationFrame].concat(equipmentFrames), compositeKey)
        : null;
      if (compositeTexture) {
        playerDrawn = this.drawActorTexture(compositeTexture, box, {
          alpha: 1,
          flipX: Number(player.facing || 1) < 0,
