...
${header}
  ...
  <main id="main" class="project-demo-wrapper-main" aria-label="${escapeHtml(definition.title)} interactive demo">
    <div class="project-demo-wrapper-frame">
      <iframe id="${escapeHtml(frameId)}" class="project-demo-wrapper-iframe" src="${escapeHtml(definition.rawPath)}" title="${escapeHtml(definition.title)} interactive demo" loading="eager" allowfullscreen></iframe>
      <script>
        (() => {
          const frame = document.getElementById(${JSON.stringify(frameId)});
          const frameContainer = frame ? frame.closest('.project-demo-wrapper-frame') : null;
          const frameMain = frame ? frame.closest('.project-demo-wrapper-main') : null;
          ...
          const clearMobileHeight = () => {
            ...
            [frameMain, frameContainer, frame].forEach((element) => {
          ...
          const measureFrame = () => {
            resizeFrame = 0;
            if (!frame || !frameContainer || !frameMain || !mobileQuery.matches) return;

            try {
              const frameDocument = frame.contentDocument;
              const frameBody = frameDocument && frameDocument.body;
              const frameRoot = frameDocument && frameDocument.documentElement;
              ...
              const height = String(lastHeight) + 'px';
          ...
          const scheduleMeasurement = () => {
            if (!mobileQuery.matches || resizeFrame) return;
          ...
          if (suffix) frame.src = ${JSON.stringify(definition.rawPath)} + suffix;
